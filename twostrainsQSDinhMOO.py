...
## Two strains with QS-mediated decay, multi-objective
solver = KenCarp5(root_finder=dfx.VeryChord(rtol=1e-3, atol=1e-6, norm=optx.rms_norm))
term = MultiTerm(ODETerm(explicit_term), ODETerm(twostrainsQSDinhlinxpODE_wrapped))
stepsize_controller = PIDController(rtol=1e-3, atol=1e-6, pcoeff=0.2, icoeff=0.5)
max_steps = 8192

@equinox.filter_jit
def twoStrainQSDinhMultiObj(hPR, cPR, Y00, r0, N0, tmax, scalefactor):
    # Initial conditions
...
def W_twoStrainQSDinhMultiObj(x, args):
    ...
    cost, output, _, _, _, _, _ = twoStrainQSDinhMultiObj(hPR, cPR, Y20, r0, N0, tmax, scalefactor)
    return cost, output

## Two strains, single objective
def twoStrainQSDinhSingleObj(x, args):
    # Simulate model
    hPR, cPR, Y20, N0, tmax, scalefactor = args
    _, output = W_twoStrainQSDinhMultiObj(x, args=(hPR, cPR, Y20, N0, tmax, jnp.array([1, 1])))
    cost = - jnp.sum(output * scalefactor)

    return cost, output

## Define the problem class for two strains multiple objectives
class twoStrainQSDinhMultiObj_problem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        cost, _ = W_twoStrainQSDinhMultiObj(x, self.args)
        out['F'] = jax.device_get(cost)

## Define the problem class for two strain single objectives
class twoStrainQSDinhSingleObj_problem(ElementwiseProblem):
    def __init__(self, nvars, nobjs, lb, ub, **kwargs):
        super().__init__(n_var=nvars, n_obj=nobjs, n_constr=0,
                         xl=lb, xu=ub)
        for key, value in kwargs.items():
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        cost, output = twoStrainQSDinhSingleObj(x, self.args)
        out['F'] = jax.device_get(cost)
...
def init_arrays(params, mparams, CellY0):
    ...
    hPR = np.array([params.phie0,params.vIxS,params.vEe,
                    ...
                    params.maxG,params.kappaG,params.M0,
                    params.kH,params.hH,
                    mparams.wI_a,mparams.wU_a,mparams.wP_a,
                    mparams.nI_a,mparams.nU_a,mparams.nP_a,
                    mparams.bI_a,mparams.bU_a,mparams.bP_a,
                    ...
                    mparams.vP_a,mparams.kP_a,
                    mparams.wI_b,mparams.wU_b,mparams.wP_b,
                    mparams.nI_b,mparams.nU_b,mparams.nP_b,
                    mparams.bI_b,mparams.bU_b,mparams.bP_b,
...
TWO_STRAINS_QS_DINH_UTILS = {
    ...
    "mparams": TwostrainsQSDecayParameters,
    "ode": twostrainsQSDinhlinxpODE,
    "ode_scipy": W_twostrainsQSDinhlinxpODE,
    "ode_diffrax": twostrainsQSDinhlinxpODE_wrapped,
    "calc_objs": W_twoStrainQSDinhMultiObj,
    "sop": twoStrainQSDinhSingleObj_problem,
    "mop": twoStrainQSDinhMultiObj_problem,
