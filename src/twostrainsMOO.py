...
## Two strains, multi-objective
solver = KenCarp5(root_finder=dfx.VeryChord(rtol=1e-3, atol=1e-6, norm=optx.rms_norm))
term = MultiTerm(ODETerm(explicit_term), ODETerm(twostrainslinxpODE_wrapped))
stepsize_controller = PIDController(rtol=1e-3, atol=1e-6, pcoeff=0.2, icoeff=0.5, dcoeff=0.2)
max_steps = 8192

@equinox.filter_jit
def twoStrainMultiObj(hPR, cPR, Y00, r0, N0, tmax, scalefactor):
    # Initial conditions
...
def W_twoStrainMultiObj(x, args):
        ...
        hPR[6] = x[3] # set wE_a
        hPR[30] = x[4] # set wE_b
        cost, output, _, _, _, _, _ = twoStrainMultiObj(hPR, cPR, Y20, r0, N0, tmax, scalefactor)
        return cost, output

## Two strains, single objective
def twoStrainSingleObj(x, args):
    # Simulate model
    hPR, cPR, Y20, N0, tmax, scalefactor = args
    _, output = W_twoStrainMultiObj(x, args=(hPR, cPR, Y20, N0, tmax, jnp.array([1, 1])))
    cost = - jnp.sum(output * scalefactor)

    return cost, output

## Define the problem class for two strains multiple objectives
class twoStrainMultiObj_problem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        cost, _ = W_twoStrainMultiObj(x, self.args)
        out['F'] = jax.device_get(cost)

## Define the problem class for two strain single objectives
class twoStrainSingleObj_problem(ElementwiseProblem):
    def __init__(self, nvars, nobjs, lb, ub, **kwargs):
        super().__init__(n_var=nvars, n_obj=nobjs, n_constr=0,
                         xl=lb, xu=ub)
        for key, value in kwargs.items():
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        cost, _ = twoStrainSingleObj(x, self.args)
        out['F'] = jax.device_get(cost)
...
def init_arrays(params, mparams, CellY0):
    ...
    hPR = np.array([params.phie0,params.vIxS,params.vEe,
                    ...
                    params.wT,params.wE,params.wH,params.wR,params.wr,
                    params.oX,params.oR,
                    params.nX,params.nR,params.bX,params.uX,params.brho,params.urho,params.dymX,
                    params.maxG,params.kappaG,params.M0,
                    params.kH,params.hH])
...
TWO_STRAINS_UTILS = {
    ...
    "mparams": None,
    "ode": twostrainslinxpODE,
    "ode_scipy": W_twostrainslinxpODE,
    "ode_diffrax": twostrainslinxpODE_wrapped,
    "calc_objs": W_twoStrainMultiObj,
    "sop": twoStrainSingleObj_problem,
    "mop": twoStrainMultiObj_problem,
